...
// ★変更: AreaSelector を StatefulWidget から StatelessWidget に変更し、コールバックを追加
class AreaSelector extends StatefulWidget {
  // 選択されたエリアのリストを親に返すコールバック
  final ValueChanged<List<String>> onAreasChanged;
  // 初期選択エリアのリスト
  ...
  const AreaSelector({
    ...
    this.initialSelectedAreas = const [],
  });
...
class _AreaSelectorState extends State<AreaSelector> {
  ...
  // ★変更: selectedAreas を状態として持ち、onAreasChanged で親に通知
  final List<String> _selectedAreas = [];

  @override
  void initState() {
    super.initState();
    _selectedAreas.addAll(widget.initialSelectedAreas);
  }

  // 兵庫県内の詳細なエリアデータ（市 -> 区・町 -> 地域・丁目）
  final Map<String, Map<String, List<String>>> hyogoAreaData = {
    "神戸市 (Kobe City)": {
      "東灘区 (Higashinada Ward)": [
      ...
      "中央区 (Chuo Ward)": [
        "相生町", "磯上通", "磯辺通", "伊藤町", "江戸町", "小野柄通", "海岸通", "加納町", "京町",
      ...
      "須磨区 (Suma Ward)": [
        "青葉町", "池田町", "一ノ谷町", "板宿町", "稲葉町", "大手", "大田町", "奥須磨", "神の谷",
    ...
    "尼崎市 (Amagasaki City)": {
      "全域 (All Areas)": [
        "潮江", "神田北通", "常光寺", "立花町", "塚口本町", "東園田町", "南塚口町", "武庫之荘", "昭和通",
    ...
    "西宮市 (Nishinomiya City)": {
      "全域 (All Areas)": [
        "池田町", "上甲子園", "甲東園", "甲陽園", "高木西町", "苦楽園一番町", "鳴尾浜", "広田町", "羽衣町",
        "今津", "大社町", "瓦林町", "甲子園口", "甲子園浜", "山口町", "鷲林寺", "門戸厄神"
      ]
    },
    "宝塚市 (Takarazuka City)": {
      "全域 (All Areas)": [
    ...
    "明石市 (Akashi City)": {
      "全域 (All Areas)": [
        "魚住町西岡", "大久保町ゆりのき通", "藤江", "林崎町", "松江", "和坂", "相生町", "旭が丘", "太寺",
        "大蔵海岸通", "金ケ崎", "貴崎", "小久保", "西明石", "東野町", "本町", "山下町"
      ]
    },
    "加古川市 (Kakogawa City)": {
      "全域 (All Areas)": [
        "尾上町口里", "加古川町寺家町", "神野町", "野口町野口", "平岡町新在家", "八幡町中西条", "東神吉町", "志方町",
        "米田町", "別府町", "神吉町", "東加古川", "西神吉町"
      ]
    },
    "高砂市 (Takasago City)": {
      "全域 (All Areas)": [
        "阿弥陀", "曽根町", "高砂町", "米田町", "荒井町", "竜山", "伊保", "今市", "魚橋",
        "梅井", "北浜", "中島", "百合ヶ丘"
      ]
    },
    "姫路市 (Himeji City)": {
  ...
  // ★変更: addSelectedArea と removeSelectedArea で親のコールバックを呼び出す
  void addSelectedArea() {
    if (_selectedCity != null && _selectedWard != null && _selectedNeighborhood != null) {
      final area = '兵庫県 (Hyogo Pref.) > $_selectedCity > $_selectedWard > $_selectedNeighborhood';
      if (!_selectedAreas.contains(area)) { // _selectedAreasを使用
        setState(() {
          _selectedAreas.add(area);
          widget.onAreasChanged(_selectedAreas); // 親に通知
        });
      }
    }
  }
